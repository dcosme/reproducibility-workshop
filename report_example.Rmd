...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, cache = TRUE, dpi = 150, fig.path = "figs/") 
...
pacman::p_load(devtools, report, tidyverse, lmerTest, usmap, knitr, kableExtra, reactable, performance, install = TRUE)
...
demo_summary %>%
  ...
  select(-n, -total, -` `) %>%
  bind_rows(data.frame(value = c("Non-binary / third gender", "Prefer to self-describe"), percent = rep("0%", 2))) %>%
  arrange(value) %>%
...
demo_summary %>%
  ...
  bind_rows(data.frame(value = c("American Indian or Alaskan Native", "Native Hawaiian or Other Pacific Islander"), percent = rep("0%", 2))) %>%
...
messages %>%
  ...
  scale_fill_manual(values = palette_relevance) +
  scale_color_manual(values = palette_relevance) +
  labs(x = "") +
  coord_flip() +
  plot_aes +
